...
public class FlightStatisticsController {
    ...
    public ResponseEntity<FlightStatisticsDto> getStatistics(@RequestParam String airportIATACode, @RequestParam String date) throws InvalidAirportIATACodeException, CargoEntityNotFoundException, FlightEntityNotFoundException {
        return ResponseEntity.ok(flightStatisticsMapper.mapToFlightStatisticsDto(dbFlightStatisticsService.getStatistics(airportIATACodeValidator.validateAirportIATACode(airportIATACode), dateParser.parseDate(date))));
    }
}